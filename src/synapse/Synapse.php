...
namespace synapse {
	...
	if(\Phar::running(true) !== ""){
		...
		@define('synapse\PATH', \getcwd() . DIRECTORY_SEPARATOR);
	}
	...
	if(!class_exists("ClassLoader", false)){
		require_once(\synapse\PATH . "src/spl/ClassLoader.php");
		require_once(\synapse\PATH . "src/spl/BaseClassLoader.php");
		require_once(\synapse\PATH . "src/synapse/CompatibleClassLoader.php");
	}

	$autoloader = new CompatibleClassLoader();
	$autoloader->addPath(\synapse\PATH . "src");
	$autoloader->addPath(\synapse\PATH . "src" . DIRECTORY_SEPARATOR . "spl");
	...
	define('synapse\ANSI', Terminal::hasFormattingCodes());

	if(!file_exists(\synapse\DATA)){
		mkdir(\synapse\DATA, 0777, true);
	}

	//Logger has a dependency on timezone, so we'll set it to UTC until we can get the actual timezone.
	date_default_timezone_set("UTC");

	$logger = new MainLogger(\synapse\DATA . "server.log", true);
	...
	function cleanPath($path){
		return rtrim(str_replace(["\\", ".php", "phar://", rtrim(str_replace(["\\", "phar://"], ["/", ""], \synapse\PATH), "/"), rtrim(str_replace(["\\", "phar://"], ["/", ""], \synapse\PLUGIN_PATH), "/")], ["/", "", "", "", ""], $path), "/");
	}

	$errors = 0;

	if(php_sapi_name() !== "cli"){
		$logger->critical("You must run synapse-MP using the CLI.");
		++$errors;
	}
	...
	if(file_exists(\synapse\PATH . ".git/refs/heads/master")){ //Found Git information!
		define('synapse\GIT_COMMIT', strtolower(trim(file_get_contents(\synapse\PATH . ".git/refs/heads/master"))));
	}else{ //Unknown :(
		define('synapse\GIT_COMMIT', str_repeat("00", 20));
	}
